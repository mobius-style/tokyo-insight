...
_EXEC_KW = ("教育長", "本部長", "局長", "次長", "担当部長", "部長", "課長",
            "館長", "所長", "技監", "監", "参事", "管理者", "知事", "室長",
            "理事者", "議事部長", "市場長", "出納長", "管理局長")
# 本会議 questioners are labelled by seat number, e.g. 百十九番（村松一希君）.
_RE_SEAT = re.compile(r"^[一二三四五六七八九十百千〇零\d]+番")
...
def _resolve(label: str, members: List[Dict], execs: List[Dict]) -> Tuple[str, str, bool]:
    ...
    # No roster match (e.g. 予算特別委員会 総括質疑 pages have no roster) — resolve
    # by the title carried in the speaker label itself.
    if any(k in lab for k in _OFFICER_KW):          # 委員長 / 副委員長 / 議長
        return lab, "officer", False
    if any(k in lab for k in _EXEC_KW):             # 局長 / 知事 / 室長 / 部長 …
        return lab, "executive", True
    if _RE_SEAT.match(lab):                         # 本会議 questioner by seat number
        return lab, "assembly_member", True
    if lab.endswith(("委員", "議員", "理事")):       # bare member in a roster-less page
        return lab, "assembly_member", True
